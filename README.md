...
- The [`es_lib`](https://github.com/IEver3st/es_lib) resource (`@es_lib/init.lua` is a hard dependency in `fxmanifest.lua`).
...
2. Make sure `es_lib` is started before this resource.
3. Add to `server.cfg`:
...
| `Permissions` | ACE permission names (`es_admin.all`, `es_admin.world`, and `weather.*` action permissions) |
...
- **External NUI assets** — the editor loads map tiles from `https://s.rsg.sc/sc/images/games/GTAV/map/game/{z}/{x}/{y}.jpg` and Google Fonts (`Outfit`, `Share+Tech+Mono`) from their CDNs.
...
- **Dependency** — `es_lib` must be present and started; notifications and `lib.require` depend on it.
